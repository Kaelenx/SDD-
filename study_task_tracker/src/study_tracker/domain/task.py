"""Domain model and validation rules for study tasks."""
...
def normalize_notes(notes: str | None) -> str:
    ...
    if len(normalized) > 240:
        raise ValidationError("notes must be at most 240 characters")
    return normalized


def normalize_estimated_hours(value: Any | None) -> float:
    if value is None or value == "":
        return 1.0
    if isinstance(value, bool):
        raise ValidationError("estimated_hours must be a number")
    try:
        hours = float(value)
    except (TypeError, ValueError) as exc:
        raise ValidationError("estimated_hours must be a number") from exc
    if hours < MIN_ESTIMATED_HOURS or hours > MAX_ESTIMATED_HOURS:
        raise ValidationError("estimated_hours must be between 0.25 and 40")
...
class StudyTask:
    ...
    def __post_init__(self) -> None:
        ...
        object.__setattr__(self, "notes", normalize_notes(self.notes))
        object.__setattr__(self, "estimated_hours", normalize_estimated_hours(self.estimated_hours))
    ...
    def new(
        ...
        return cls(
            ...
            estimated_hours=normalize_estimated_hours(estimated_hours),
        )
    ...
    def from_dict(cls, raw: dict[str, Any]) -> "StudyTask":
        ...
        return cls(
            ...
            notes=raw.get("notes", ""),
            estimated_hours=raw.get("estimated_hours", 1.0),
        )
