...
from ..application.task_service import StudyTaskService
from ..domain.task import StudyTrackerError
from ..infrastructure.json_repository import JsonTaskRepository
from .serializers import JsonObject, course_summary_to_dict, summary_to_dict
    ...
    class StudyTrackerRequestHandler(SimpleHTTPRequestHandler):
        ...
        def do_GET(self) -> None:
            ...
            if parsed.path == "/api/courses":
                self._handle_json(lambda service: [course_summary_to_dict(item) for item in service.course_summaries()])
                return
            if parsed.path == "/api/export":
                self._handle_json(
                    lambda service: {
                        "summary": summary_to_dict(service.summary()),
                        "courses": [course_summary_to_dict(item) for item in service.course_summaries()],
        ...
        def do_PATCH(self) -> None:
            parsed = urlparse(self.path)
            if parsed.path == "/api/tasks/bulk-complete":
                body = self._read_json_body()
                if body is None:
                    return
                task_ids = body.get("task_ids")
                if not isinstance(task_ids, list):
                    self._send_json({"error": "task_ids must be an array"}, status=HTTPStatus.BAD_REQUEST)
                    return
                self._handle_json(lambda service: [task.to_dict() for task in service.complete_tasks(task_ids)])
                return
